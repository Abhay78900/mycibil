  ...
  return (
    <PartnerLayout partner={partner}>
      <div className="mb-6 lg:mb-8">
        <h1 className="text-2xl lg:text-3xl font-bold text-foreground">Welcome, {partner?.name}</h1>
        <p className="text-muted-foreground mt-1">Manage your clients and generate reports</p>
      </div>

      <Card className="mb-6 lg:mb-8 bg-gradient-to-r from-secondary/20 to-secondary/5 border-secondary/30">
        <CardContent className="p-4 lg:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xl lg:text-2xl font-mono font-bold text-foreground">{partner?.franchise_id}</p>
            <p className="text-sm text-muted-foreground mt-2">Commission Rate: <span className="font-bold text-success">{partner?.commission_rate}%</span></p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
        <WalletCard balance={Number(partner?.wallet_balance || 0)} effectiveReportCount={getEffectiveReportCount(Number(partner?.wallet_balance || 0), Number(partner?.report_count || 0))} isReportCountMode={isReportCountMode} onAddFunds={() => navigate('/partner/wallet')} />
        <StatsCard title="Total Reports" value={stats.totalReports} icon={FileText} />
        <StatsCard title="Total Clients" value={stats.totalClients} icon={Users} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-primary" />Recent Reports</CardTitle>
            <Button variant="ghost" size="sm" onClick={() => navigate('/partner/reports')}>View All</Button>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {recentReports.map((report) => (
                <div key={report.id} className="flex items-center justify-between p-3 lg:p-4 bg-accent/50 rounded-lg">
                  <div className="min-w-0"><p className="font-medium text-foreground truncate">{report.full_name}</p><p className="text-sm text-muted-foreground">{report.pan_number}</p></div>
                  <div className="text-right ml-3"><p className="font-bold text-primary">{report.average_score || 'N/A'}</p><p className={`text-xs font-medium ${report.report_status === 'unlocked' ? 'text-success' : 'text-warning'}`}>{report.report_status?.toUpperCase()}</p></div>
                </div>
              ))}
              {recentReports.length === 0 && (<p className="text-center text-muted-foreground py-8">No reports yet</p>)}
            </div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5 text-primary" />Quick Generate Report</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={handleGenerateReport} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2"><Label htmlFor="fullName">Full Name *</Label><Input id="fullName" value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} placeholder="Client's full name" required /></div>
                <div className="space-y-2"><Label htmlFor="panNumber">PAN Number *</Label><Input id="panNumber" value={formData.panNumber} onChange={(e) => setFormData({ ...formData, panNumber: e.target.value.toUpperCase() })} placeholder="ABCDE1234F" maxLength={10} required /></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <RadioGroup value={formData.gender} onValueChange={(value) => setFormData({ ...formData, gender: value })} className="flex gap-4 pt-2">
                    <div className="flex items-center space-x-2"><RadioGroupItem value="male" id="dash-male" /><Label htmlFor="dash-male" className="font-normal cursor-pointer">Male</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="female" id="dash-female" /><Label htmlFor="dash-female" className="font-normal cursor-pointer">Female</Label></div>
                  </RadioGroup>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {bureauOptions.map((bureau) => (
                    <Button key={bureau.id} type="button" variant={selectedBureaus.includes(bureau.id) ? "default" : "outline"} size="sm" onClick={() => { setSelectedBureaus(prev => prev.includes(bureau.id) ? prev.filter(id => id !== bureau.id) : [...prev, bureau.id]); }}>
                      {bureau.label}
                    </Button>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <span className="text-sm text-muted-foreground">{isReportCountMode ? `${bureauCount} report(s) will be deducted` : `Total: ₹${totalCost}`}</span>
                <Button type="submit" disabled={isSubmitting || hasInsufficientBalance}>
                  {isSubmitting ? (<><Loader2 className="w-4 h-4 mr-2 animate-spin" />Generating...</>) : (<><FileText className="w-4 h-4 mr-2" />Generate</>)}
                </Button>
              </div>
              {hasInsufficientBalance && (<p className="text-sm text-destructive">{isReportCountMode ? 'Insufficient reports' : 'Insufficient balance'}</p>)}
            </form>
          </CardContent>
        </Card>
      </div>
