...
export default function CreditReportPage() {
  ...
  const handleDownload = async (bureau: string) => {
    ...
    try {
      ...
      // Clone the visible report content
      const element = reportContentRef.current.cloneNode(true) as HTMLElement;
      
      // Remove any motion/animation wrappers artifacts
      element.style.opacity = '1';
      element.style.transform = 'none';
      
      // Ensure all child elements are visible
      element.querySelectorAll('[style*="opacity"]').forEach((el) => {
        (el as HTMLElement).style.opacity = '1';
      });
      element.querySelectorAll('[style*="transform"]').forEach((el) => {
        (el as HTMLElement).style.transform = 'none';
      });

      const filename = `${config.name}_Report_${report.pan_number}_${format(new Date(), 'yyyyMMdd')}.pdf`;
