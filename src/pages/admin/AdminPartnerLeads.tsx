...
import { Search, Filter, Eye, Phone, Mail, MapPin, Calendar, Users, TrendingUp, UserCheck, Clock } from 'lucide-react';
...
import { PartnerLead, LEAD_STATUS_LABELS, LEAD_STATUS_COLORS } from '@/types/partnerLead';
...
export default function AdminPartnerLeads() {
  ...
  const filteredLeads = leads.filter(lead => {
    const matchesSearch = lead.full_name.toLowerCase().includes(searchQuery.toLowerCase()) || lead.email.toLowerCase().includes(searchQuery.toLowerCase()) || lead.mobile.includes(searchQuery) || lead.city.toLowerCase().includes(searchQuery.toLowerCase());
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search by name, email, mobile, city..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="w-full sm:w-[200px]"><Filter className="w-4 h-4 mr-2" /><SelectValue placeholder="Filter by status" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All Statuses</SelectItem>
                  {Object.entries(LEAD_STATUS_LABELS).map(([value, label]) => (<SelectItem key={value} value={value}>{label}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
                  ...
                  {visibleLeads.length === 0 ? (
                    ...
                    visibleLeads.map((lead) => (
                      <TableRow key={lead.id}>
                        ...
                        <TableCell><div className="flex items-center gap-1 text-sm"><Calendar className="w-3 h-3" />{format(new Date(lead.created_at), 'dd MMM yyyy')}</div></TableCell>
            ...
            {filteredLeads.length > visibleCount && (
              <div className="mt-4 text-center"><Button variant="outline" onClick={() => setVisibleCount(prev => prev + 10)}>View More ({filteredLeads.length - visibleCount} remaining)</Button></div>
            )}
          </CardContent>
        </Card>
      </div>
